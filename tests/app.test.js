...
const loan_1 = __importDefault(require("../src/models/loan"));
...
describe('workflow backend - typescript', () => {
    ...
    it('builds with default options', async () => {
        const aggregateSpy = jest.spyOn(loan_1.default, 'aggregate').mockResolvedValueOnce([]);
        const findSpy = jest
            .spyOn(loan_1.default, 'find')
            .mockResolvedValueOnce([])
            .mockReturnValueOnce(withSort([]));
        const defaultApp = (0, app_1.buildApp)();
        const health = await (0, supertest_1.default)(defaultApp).get('/health');
        expect(health.status).toBe(200);
        const metrics = await (0, supertest_1.default)(defaultApp).get('/api/dashboard/metrics');
        expect(metrics.status).toBe(200);
        aggregateSpy.mockRestore();
        findSpy.mockRestore();
    });
    it('validates helper functions', () => {
        expect((0, app_1.extractMentions)('hello @approver and @ops')).toEqual(['approver', 'ops']);
        expect((0, app_1.extractMentions)('no mention')).toEqual([]);
        expect((0, app_1.allChecklistChecksPassed)(validChecklist)).toBe(true);
        expect((0, app_1.allChecklistChecksPassed)({ ...validChecklist, idVerified: false })).toBe(false);
        expect((0, app_1.allChecklistChecksPassed)(undefined)).toBe(false);
    });
    it('creates instruction, validates checklist and priority', async () => {
        loanModel.create.mockResolvedValue(mockLoanDoc({ priority: 'urgent' }));
        const invalidChecklist = await (0, supertest_1.default)(app)
        ...
        const invalidPriority = await (0, supertest_1.default)(app)
            .post('/api/loans/instructions')
        ...
        expect(loanModel.create).toHaveBeenCalledWith(expect.objectContaining({
            instructionLocked: true,
            status: 'awaiting_disbursement',
            assignee: null,
            priority: 'urgent'
        }));
        expect(loanModel.create.mock.calls[0][0].statusHistory[0]).toEqual(expect.objectContaining({ status: 'awaiting_disbursement', changedBy: 'FA1' }));
    });
    it('lists queue variants with priority ordering', async () => {
        ...
        const unassigned = await (0, supertest_1.default)(app).get('/api/loans/queue?status=unassigned');
        ...
        const filtered = await (0, supertest_1.default)(app).get('/api/loans/queue?status=processing');
        expect(filtered.status).toBe(200);
        expect(loanModel.find).toHaveBeenNthCalledWith(2, { status: 'processing' });
        const all = await (0, supertest_1.default)(app).get('/api/loans/queue?status=all');
        expect(all.status).toBe(200);
        expect(loanModel.find).toHaveBeenNthCalledWith(3, {});
    });
    it('handles claim lifecycle constraints', async () => {
        const missing = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({});
        expect(missing.status).toBe(400);
        loanModel.findById.mockResolvedValueOnce(null);
        const notFound = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({ officerName: 'Alice' });
        expect(notFound.status).toBe(404);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ status: 'completed' }));
        const invalidState = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({ officerName: 'Alice' });
        expect(invalidState.status).toBe(409);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ assignee: 'Bob' }));
        const conflict = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({ officerName: 'Alice' });
        expect(conflict.status).toBe(409);
        const loan = mockLoanDoc({ statusHistory: [] });
        loanModel.findById.mockResolvedValueOnce(loan);
        const success = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({ officerName: 'Alice' });
        ...
        const idempotent = await (0, supertest_1.default)(app).post('/api/loans/1/claim').send({ officerName: 'Alice' });
    ...
    it('keeps instruction locked for non-approver and unlock path on action_required', async () => {
        loanModel.findById.mockResolvedValueOnce(null);
        const notFound = await (0, supertest_1.default)(app).patch('/api/loans/1/instruction').send({ actorRole: 'final_approver' });
        expect(notFound.status).toBe(404);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc());
        const wrongRole = await (0, supertest_1.default)(app)
            .patch('/api/loans/1/instruction')
            .send({ actorRole: 'disbursement_officer', instruction: { beneficiaryName: 'X' } });
        expect(wrongRole.status).toBe(403);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ status: 'processing' }));
        const locked = await (0, supertest_1.default)(app)
            .patch('/api/loans/1/instruction')
            .send({ actorRole: 'final_approver', instruction: { beneficiaryName: 'X' } });
        expect(locked.status).toBe(403);
        const editable = mockLoanDoc({ status: 'action_required', assignee: 'A' });
        ...
        const unchanged = await (0, supertest_1.default)(app)
            .patch('/api/loans/1/instruction')
    ...
    it('returns disbursement package and status breadcrumbs', async () => {
        loanModel.findById.mockResolvedValueOnce(null);
        const missingPackage = await (0, supertest_1.default)(app).get('/api/loans/1/package');
        expect(missingPackage.status).toBe(404);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ documents: ['agreement.pdf'] }));
        const packageRes = await (0, supertest_1.default)(app).get('/api/loans/1/package');
        ...
        loanModel.findById.mockResolvedValueOnce(null);
        const missingBreadcrumbs = await (0, supertest_1.default)(app).get('/api/loans/1/status-breadcrumbs');
        ...
        const breadcrumbs = await (0, supertest_1.default)(app).get('/api/loans/1/status-breadcrumbs');
    ...
    it('handles comments, return, and hold authorization', async () => {
        const invalidComment = await (0, supertest_1.default)(app).post('/api/loans/1/comments').send({ author: '', message: '' });
        expect(invalidComment.status).toBe(400);
        loanModel.findById.mockResolvedValueOnce(null);
        const missingComment = await (0, supertest_1.default)(app)
        ...
        const createdComment = await (0, supertest_1.default)(app)
            .post('/api/loans/1/comments')
        ...
        const missingReason = await (0, supertest_1.default)(app).post('/api/loans/1/return').send({});
        expect(missingReason.status).toBe(400);
        loanModel.findById.mockResolvedValueOnce(null);
        const missingReturn = await (0, supertest_1.default)(app).post('/api/loans/1/return').send({ reason: 'Account closed' });
        expect(missingReturn.status).toBe(404);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ assignee: 'Alice' }));
        const forbiddenReturn = await (0, supertest_1.default)(app)
        ...
        const returned = await (0, supertest_1.default)(app)
            .post('/api/loans/1/return')
        ...
        const returnWithoutOfficer = mockLoanDoc({ assignee: null, status: 'processing' });
        loanModel.findById.mockResolvedValueOnce(returnWithoutOfficer);
        const returnedWithoutOfficer = await (0, supertest_1.default)(app)
            .post('/api/loans/1/return')
        ...
        loanModel.findById.mockResolvedValueOnce(null);
        const missingHold = await (0, supertest_1.default)(app).patch('/api/loans/1/hold').send({ officerName: 'Alice' });
        expect(missingHold.status).toBe(404);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ assignee: 'Alice' }));
        const forbiddenHold = await (0, supertest_1.default)(app).patch('/api/loans/1/hold').send({ officerName: 'Bob' });
        expect(forbiddenHold.status).toBe(403);
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ assignee: 'Alice' }));
        const missingOfficerHold = await (0, supertest_1.default)(app).patch('/api/loans/1/hold').send({});
        expect(missingOfficerHold.status).toBe(403);
        const holdLoan = mockLoanDoc({ assignee: 'Alice', status: 'processing' });
        loanModel.findById.mockResolvedValueOnce(holdLoan);
        const hold = await (0, supertest_1.default)(app).patch('/api/loans/1/hold').send({ officerName: 'Alice' });
    ...
    it('completes loan with mandatory transaction reference and ownership checks', async () => {
        const missingTransactionRef = await (0, supertest_1.default)(app)
            .post('/api/loans/1/complete')
            .send({ officerName: 'Alice' });
        expect(missingTransactionRef.status).toBe(400);
        loanModel.findById.mockResolvedValueOnce(null);
        const missingLoan = await (0, supertest_1.default)(app)
            .post('/api/loans/1/complete')
        ...
        loanModel.findById.mockResolvedValueOnce(mockLoanDoc({ assignee: 'Alice' }));
        const missingOfficer = await (0, supertest_1.default)(app)
            .post('/api/loans/1/complete')
            .send({ transactionReference: 'FT123' });
        expect(missingOfficer.status).toBe(403);
        const completeLoan = mockLoanDoc({ assignee: 'Alice', status: 'processing' });
        loanModel.findById.mockResolvedValueOnce(completeLoan);
        const complete = await (0, supertest_1.default)(app)
            .post('/api/loans/1/complete')
            .send({
            officerName: 'Alice',
    ...
    it('computes supervisor capacity and dashboard metrics', async () => {
        ...
        const capacity = await (0, supertest_1.default)(app).get('/api/supervisor/capacity');
        ...
        const metrics = await (0, supertest_1.default)(app).get('/api/dashboard/metrics?staleHours=48');
        expect(metrics.status).toBe(200);
        ...
        expect(metrics.body.staleApplications).toEqual(expect.arrayContaining([expect.objectContaining({ id: 'stale-1', loanReference: 'LN-ST-1' })]));
        loanModel.aggregate.mockResolvedValueOnce([]);
        loanModel.find.mockResolvedValueOnce([]).mockReturnValueOnce(withSort([]));
        const empty = await (0, supertest_1.default)(app).get('/api/dashboard/metrics');
    ...
    it('handles validation and unknown server errors', async () => {
        ...
        const validation = await (0, supertest_1.default)(app)
            .post('/api/loans/instructions')
        ...
        const unknown = await (0, supertest_1.default)(app).get('/api/loans/queue?status=unassigned');
        expect(unknown.status).toBe(500);
    });
    it('enforces rate limiting', async () => {
        const limitedModel = createMockModel();
        limitedModel.find.mockReturnValue(withSort([]));
        const limitedApp = (0, app_1.buildApp)({
            loanModel: limitedModel,
            rateLimit: { windowMs: 1000, maxRequests: 1 }
        });
        const first = await (0, supertest_1.default)(limitedApp).get('/api/loans/queue?status=unassigned');
        expect(first.status).toBe(200);
        const second = await (0, supertest_1.default)(limitedApp).get('/api/loans/queue?status=unassigned');
        expect(second.status).toBe(429);
    });
});
describe('loan model schema', () => {
    it('enforces required fields and enum constraints', () => {
        const invalid = new loan_1.default({
            instruction: {
        ...
        const valid = new loan_1.default({
            instruction: {
                beneficiaryName: 'John',
                accountNumber: '111',
                bankCode: '001',
